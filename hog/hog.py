    ...
    def say(score0,score1):
        if who == 0 :
            if score0 - last_score >running_high:
                print(score0-last_score,"point(s)! That's a record gain for Player",str(who)+"!")
            return announce_highest(who,score0,max(score0 - last_score,running_high))
        if who == 1:
            if score1 - last_score > running_high:
                print(score1 - last_score, "point(s)! That's a record gain for Player",str(who)+"!")
...
def make_averaged(original_function, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 8
    def averaged_print(*args):
        sum=0.0
        for i in range(trials_count):
            sum = sum + original_function(*args)
        return sum/trials_count
    return averaged_print
    # END PROBLEM 8
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if picky_piggy_strategy(score,opponent_score,cutoff,num_rolls) == 0 :
        return 0
    if picky_piggy(opponent_score)+score == opponent_score:
        return 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
